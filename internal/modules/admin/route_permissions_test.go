...
func TestAdminBatchRoleRoutePermissions(t *testing.T) {
	app := buildRoleProtectedApp(
		http.MethodPost,
	...
	req := httptest.NewRequest(http.MethodPost, "/api/admin/users/batch-role", nil)
	req.Header.Set("X-User-ID", "1001")
	req.Header.Set("X-Role", roleAdmin)
	resp, err := app.Test(req)
	if err != nil {
...
func TestAdminBatchAllowCNMysekaiRoutePermissions(t *testing.T) {
	app := buildRoleProtectedApp(
		http.MethodPost,
		"/api/admin/users/batch-allow-cn-mysekai",
		[]string{roleAdmin, roleSuperAdmin},
		func(c fiber.Ctx) error { return c.SendStatus(fiber.StatusNoContent) },
	)

	req := httptest.NewRequest(http.MethodPost, "/api/admin/users/batch-allow-cn-mysekai", nil)
	req.Header.Set("X-User-ID", "1001")
	req.Header.Set("X-Role", roleUser)
	resp, err := app.Test(req)
	if err != nil {
