...
func TestNormalizeRole(t *testing.T) {
	t.Run("empty defaults to user", func(t *testing.T) {
		if got := adminCoreModule.NormalizeRole(""); got != roleUser {
			t.Fatalf("NormalizeRole(\"\") = %q, want %q", got, roleUser)
		}
	})

	t.Run("trim and lowercase", func(t *testing.T) {
		if got := adminCoreModule.NormalizeRole("  SUPER_Admin "); got != roleSuperAdmin {
...
func TestIsValidRole(t *testing.T) {
	if !adminCoreModule.IsValidRole(roleUser) || !adminCoreModule.IsValidRole(roleAdmin) || !adminCoreModule.IsValidRole(roleSuperAdmin) {
		t.Fatalf("expected built-in roles to be valid")
	}
	if adminCoreModule.IsValidRole("owner") {
