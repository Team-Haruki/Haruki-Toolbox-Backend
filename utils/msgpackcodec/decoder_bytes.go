...
func (d *cursor) decodeValue(depth int) (any, error) {
...
func (d *cursor) decodeMap(n int, depth int) (*orderedmap.OrderedMap, error) {
	// A key and value each consume at least one byte. Validate before allocation.
	if n < 0 || n > (len(d.data)-d.off)/2 {
		return nil, fmt.Errorf("map length %d exceeds remaining bytes", n)
	}
	// Bound speculative capacity even when duplicate keys collapse a large map.
	om := orderedmap.NewSize(min(n, 4096))
	for i := range n {
...
func (d *cursor) decodeMapKey(depth int) (string, error) {
	b, err := d.readByte()
	if err != nil {
...
func (d *cursor) decodeArray(n int, depth int) ([]any, error) {
...
func (d *cursor) decodeString(n int) (string, error) {
...
func (d *cursor) decodeExtPayload(size int) (any, error) {
	_, err := d.readByte()
	if err != nil {
