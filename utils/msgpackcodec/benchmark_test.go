...
func BenchmarkDecodeOrderedSmall(b *testing.B) {
	data := benchmarkOrderedMapData(b, 8, false)
	b.ReportAllocs()
	b.SetBytes(int64(len(data)))
	for i := 0; i < b.N; i++ {
		if _, err := DecodeOrdered(data); err != nil {
			b.Fatalf("DecodeOrdered: %v", err)
		}
	}
}

func BenchmarkDecodeOrderedNested(b *testing.B) {
...
func BenchmarkDecodeOrderedLargeArray(b *testing.B) {
