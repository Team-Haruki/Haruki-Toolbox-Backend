...
func TestNewSuiteMatchesHarvestSchema(t *testing.T) {
	suite, err := New(map[string]string{"cn": "../../data/suite_user_cn_6.4.0.avsc"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err = New(map[string]string{"cn": "../../data/suite_user.avsc"}); err == nil {
...
func TestSchemaFileReplacementChangesCacheAndKeepsLiveInstance(t *testing.T) {
	...
	first, err := New(map[string]string{"cn": path})
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := New(map[string]string{"cn": path})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInvalidSchemasReturnErrors(t *testing.T) {
	...
	if _, err := New(map[string]string{"cn": filepath.Join(t.TempDir(), "missing.avsc")}); err == nil {
		t.Fatal("missing file accepted")
	}
	var disabled *Restorer
