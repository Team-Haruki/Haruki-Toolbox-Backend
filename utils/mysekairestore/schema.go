...
// Restorer owns immutable schemas loaded at startup. Nil and empty configuration disable restoration.
type Restorer struct{ regions map[string]layout }

// New accepts AVSC file paths per region. Files are read once; replacement takes
// effect on restart and changes the response cache fingerprint even at the same path.
func New(paths map[string]string) (*Restorer, error) {
	r := &Restorer{regions: make(map[string]layout)}
...
func (r *Restorer) Fingerprint(server string) string {
	if r == nil {
		return ""
	}
