...
func TestRegionProfilesAndBothDocumentShapes(t *testing.T) {
	...
	disabled, _ := NewMysekai(map[string]string{})
	if disabled.Fingerprint("cn") != "" {
		t.Fatal("explicit disable ignored")
	}
	defaults, _ := NewMysekai(nil)
...
func testRestorer(t *testing.T) *MysekaiRestorer {
	t.Helper()
	r, err := NewMysekai(map[string]string{"cn": "testdata/cn-6.4.0.harvest_map.avsc"})
	if err != nil {
		t.Fatal(err)
	}
	return r
