...
func createTestRestorer(t *testing.T) *SuiteRestorer {
	t.Helper()
	...
	r, err := NewSuiteFromDefinitions(definitions)
	if err != nil {
		t.Fatalf("NewSuiteFromDefinitions: %v", err)
	}
	return r
}
...
func TestNewFromDefinitionsInvalidNestedDefinition(t *testing.T) {
	_, err := NewSuiteFromDefinitions(map[string][]any{
