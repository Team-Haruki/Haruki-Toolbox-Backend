...
func (_q *OAuthTokenQuery) QueryClient() *OAuthClientQuery {
...
func (_q *OAuthTokenQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *OAuthTokenQuery) FirstIDX(ctx context.Context) int {
...
func (_q *OAuthTokenQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *OAuthTokenQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *OAuthTokenQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *OAuthTokenQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *OAuthTokenQuery) WithClient(opts ...func(*OAuthClientQuery)) *OAuthTokenQuery {
	query := (&OAuthClientClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OAuthTokenQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*OAuthToken, init func(*OAuthToken), assign func(*OAuthToken, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_oauth_tokens == nil {
			continue
		}
		fk := *nodes[i].user_oauth_tokens
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_oauth_tokens" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *OAuthTokenQuery) loadClient(ctx context.Context, query *OAuthClientQuery, nodes []*OAuthToken, init func(*OAuthToken), assign func(*OAuthToken, *OAuthClient)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OAuthToken)
	for i := range nodes {
		if nodes[i].oauth_client_tokens == nil {
			continue
		}
		fk := *nodes[i].oauth_client_tokens
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "oauth_client_tokens" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
