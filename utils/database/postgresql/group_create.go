...
func (_c *GroupCreate) SetGroup(v string) *GroupCreate {
...
func (_c *GroupCreate) AddGroupListIDs(ids ...int) *GroupCreate {
	_c.mutation.AddGroupListIDs(ids...)
	return _c
}

// AddGroupList adds the "group_list" edges to the GroupList entity.
func (_c *GroupCreate) AddGroupList(v ...*GroupList) *GroupCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddGroupListIDs(ids...)
}
...
func (_c *GroupCreate) check() error {
	if _, ok := _c.mutation.Group(); !ok {
		return &ValidationError{Name: "group", err: errors.New(`postgresql: missing required field "Group.group"`)}
	}
