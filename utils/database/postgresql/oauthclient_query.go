...
func (_q *OAuthClientQuery) QueryAuthorizations() *OAuthAuthorizationQuery {
...
func (_q *OAuthClientQuery) QueryTokens() *OAuthTokenQuery {
...
func (_q *OAuthClientQuery) WithAuthorizations(opts ...func(*OAuthAuthorizationQuery)) *OAuthClientQuery {
	query := (&OAuthAuthorizationClient{config: _q.config}).Query()
...
func (_q *OAuthClientQuery) WithTokens(opts ...func(*OAuthTokenQuery)) *OAuthClientQuery {
	query := (&OAuthTokenClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OAuthClientQuery) loadAuthorizations(ctx context.Context, query *OAuthAuthorizationQuery, nodes []*OAuthClient, init func(*OAuthClient), assign func(*OAuthClient, *OAuthAuthorization)) error {
	...
	for _, n := range neighbors {
		fk := n.oauth_client_authorizations
		if fk == nil {
			return fmt.Errorf(`foreign-key "oauth_client_authorizations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "oauth_client_authorizations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *OAuthClientQuery) loadTokens(ctx context.Context, query *OAuthTokenQuery, nodes []*OAuthClient, init func(*OAuthClient), assign func(*OAuthClient, *OAuthToken)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*OAuthClient)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.oauth_client_tokens
		if fk == nil {
			return fmt.Errorf(`foreign-key "oauth_client_tokens" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "oauth_client_tokens" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
