...
// QueryGroupList chains the current query on the "group_list" edge.
func (_q *GroupQuery) QueryGroupList() *GroupListQuery {
	query := (&GroupListClient{config: _q.config}).Query()
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupList: _q.withGroupList.Clone(),
...
// WithGroupList tells the query-builder to eager-load the nodes that are connected to
// the "group_list" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithGroupList(opts ...func(*GroupListQuery)) *GroupQuery {
	query := (&GroupListClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupList = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withGroupList; query != nil {
		if err := _q.loadGroupList(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupList = []*GroupList{} },
			func(n *Group, e *GroupList) { n.Edges.GroupList = append(n.Edges.GroupList, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadGroupList(ctx context.Context, query *GroupListQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupList)) error {
	...
	query.withFKs = true
	query.Where(predicate.GroupList(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupListColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.group_group_list
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_group_list" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_group_list" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
