...
func (_q *UserQuery) QuerySocialPlatformInfo() *SocialPlatformInfoQuery {
	query := (&SocialPlatformInfoClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryAuthorizedSocialPlatforms chains the current query on the "authorized_social_platforms" edge.
func (_q *UserQuery) QueryAuthorizedSocialPlatforms() *AuthorizeSocialPlatformInfoQuery {
	query := (&AuthorizeSocialPlatformInfoClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryGameAccountBindings chains the current query on the "game_account_bindings" edge.
func (_q *UserQuery) QueryGameAccountBindings() *GameAccountBindingQuery {
	query := (&GameAccountBindingClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// the "social_platform_info" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSocialPlatformInfo(opts ...func(*SocialPlatformInfoQuery)) *UserQuery {
	query := (&SocialPlatformInfoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSocialPlatformInfo = query
	return _q
}

// WithAuthorizedSocialPlatforms tells the query-builder to eager-load the nodes that are connected to
// the "authorized_social_platforms" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithAuthorizedSocialPlatforms(opts ...func(*AuthorizeSocialPlatformInfoQuery)) *UserQuery {
	query := (&AuthorizeSocialPlatformInfoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAuthorizedSocialPlatforms = query
	return _q
}

// WithGameAccountBindings tells the query-builder to eager-load the nodes that are connected to
// the "game_account_bindings" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithGameAccountBindings(opts ...func(*GameAccountBindingQuery)) *UserQuery {
	query := (&GameAccountBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSocialPlatformInfo; query != nil {
		if err := _q.loadSocialPlatformInfo(ctx, query, nodes, nil,
			func(n *User, e *SocialPlatformInfo) { n.Edges.SocialPlatformInfo = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAuthorizedSocialPlatforms; query != nil {
		if err := _q.loadAuthorizedSocialPlatforms(ctx, query, nodes,
			func(n *User) { n.Edges.AuthorizedSocialPlatforms = []*AuthorizeSocialPlatformInfo{} },
			func(n *User, e *AuthorizeSocialPlatformInfo) {
				n.Edges.AuthorizedSocialPlatforms = append(n.Edges.AuthorizedSocialPlatforms, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withGameAccountBindings; query != nil {
		if err := _q.loadGameAccountBindings(ctx, query, nodes,
...
func (_q *UserQuery) loadAuthorizedSocialPlatforms(ctx context.Context, query *AuthorizeSocialPlatformInfoQuery, nodes []*User, init func(*User), assign func(*User, *AuthorizeSocialPlatformInfo)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(authorizesocialplatforminfo.FieldUserID)
	}
	query.Where(predicate.AuthorizeSocialPlatformInfo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuthorizedSocialPlatformsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadGameAccountBindings(ctx context.Context, query *GameAccountBindingQuery, nodes []*User, init func(*User), assign func(*User, *GameAccountBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.GameAccountBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GameAccountBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_game_account_bindings
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_game_account_bindings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
