...
func TestMsgpackToOrderedMapUnsupportedType(t *testing.T) {
	...
	if _, err := DecodeOrdered(data); err == nil {
		t.Fatalf("DecodeOrdered should fail for unsupported type byte")
	}
}
...
func TestMsgpackToOrderedMapRejectsTrailingBytes(t *testing.T) {
	...
	if _, err := DecodeOrdered(data); err == nil {
		t.Fatalf("DecodeOrdered should reject trailing bytes")
	}
}

func TestMsgpackToOrderedMapRequiresTopLevelMap(t *testing.T) {
	if _, err := DecodeOrdered([]byte{msgpackFixArrMin | 0x01, 0x01}); err == nil {
		t.Fatalf("DecodeOrdered should fail for a non-map top-level value")
	}
}
...
func TestMsgpackToOrderedMapDepthLimit(t *testing.T) {
	...
	if _, err := DecodeOrdered(append([]byte{msgpackFixMapMin | 0x01, msgpackFixStrMin | 0x01, 'a'}, data...)); err == nil {
		t.Fatalf("DecodeOrdered should fail when max depth is exceeded")
	}
}
...
func TestMsgpackToOrderedMapRejectsTruncatedInputs(t *testing.T) {
	...
	for _, data := range cases {
		if _, err := DecodeOrdered(data); err == nil {
			t.Fatalf("DecodeOrdered should fail for truncated input %x", data)
		}
	}
}
