...
> 状态：**实现已落地（2026-08-25），等待维护窗口执行切换**。代码侧 U0–U9 全部完成，`gofmt`／`go build`／`go vet`／`staticcheck`／`go test` 全清；剩余 U10–U12 是运维步骤，见 §7.6 runbook。§9.2 的开放问题已于 2026-08-24 全部拍板，结论并入 §9.1 与各章。所有数字均为实测；**§3.0 起已用生产全量普查数据**（2026-08-23），**§7.5 第二轮是生产 `suite` 全量副本上的离线装载**（10,928 篇，2026-08-24），测量条件见 §10。
...
| 9 | 清空 `suite_remove_keys`（含 `userProfileHonors` #6 与 `userMaterials` #13） | ← 不可逆点，见下。公开 API 对这些键从恒返 `[]` 变成真实数据，**随本步一并公告** |
...
⚠️ **不可逆点是第 9 步（清空 `suite_remove_keys`），不是第 10/11 步。** 一旦停止置空，数据量恢复到会让 MongoDB p99 爆表的水平（§1.1），此时即使 Mongo 数据还在也回不去了。因此第 9 步必须 gate 在第 8 步观察期通过，并要求显式人工确认。

⚠️ 反过来，第 9 步**提前**同样致命：只要 Mongo 还在被写入，清空 `suite_remove_keys` 会立刻把文档推到 16 MB 上限之上。
...
**M1 不通过是本方案存在的理由，不是阻断项** —— 它恰好证明"留在 MongoDB 上恢复 21 键"不可行（见 §1.1）。它真正约束的是**顺序**：§7.1 第 9 步（清空 `suite_remove_keys`）必须等到写源完全切到 PG 之后，绝不能在双写期间做。
...
#### U12 清空 `suite_remove_keys`（不可逆）

单独一次发布，gate 在 U11 的观察期之后，需要显式人工确认。

- 公开 API 对这些键从恒返 `[]` 变成真实数据，**随本步一并公告**
- 它会反转 `utils/handler/suite_restore_test.go` 里的三条断言，同一次提交改掉，别等它以失败的形式出现
- 各账号在**下次上传之前**这些键仍是 `[]`：数据从未被存储过，无法回填

⚠️ **这一步提前同样致命**：只要 MongoDB 还在被写入，清空它会立刻把文档推过 16 MB 上限。
...
| 6 | `userProfileHonors` 的配置修正何时走？ | **作为对外变更修复**，与 `userMaterials`（#13）一起并入 §7.1 第 9 步（清空 `suite_remove_keys`）—— 两者本来就只是 `suite_remove_keys` 里的条目，删掉它们就是第 9 步本身。公开 API 从恒返 `[]` 变成真实数据，随第 9 步一并公告 |
