...
> 状态：**已切换。生产自 2026-08-29 01:29 起从 PostgreSQL 读取**，当前处于第 8 步观察期（≥ 7 天）。切换**没有使用维护窗口**——双写让 PG 始终与 Mongo 同步，翻转本身只花 6 秒，上传全程未中断；第 4、5、7 步作废，详见 §7.1.2。放行闸不是 `gamedata-migrate verify`（双写引入的 float32 表示差异使它永远报 differing），而是 §7.6 的读翻转彩排：对着生产两个库 714 条响应 0 mismatch。第 9 步已按 §7.1.1 修订并全部执行：U12a（00:58，剥离下沉到 Mongo 写入路径、PG 存全量）与 U12b（02:33，放开 `userProfileHonors` 并把 18 个键加进 `allowed_keys`，公开响应键数 32→50，见 §7.1.3）。**唯一尚欠的是对外公告** —— 数据自 2026-08-29 起积累且无法回填，2,948／4,311 个 jp 账号会长期返回 `[]`。MongoDB **仍在被写入**（剥离版），作为无时间边界的回滚快照。§1.1.1／§1.1.2 是新增的实测与前提更正，其中 §1.1.2 更正了「21 键已置空」这一前提（对 cn／tw／kr 从未生效）。代码侧 U0–U9 全部完成，`gofmt`／`go build`／`go vet`／`staticcheck`／`go test` 全清；剩余 U10–U12 是运维步骤，见 §7.6 runbook。§9.2 的开放问题已于 2026-08-24 全部拍板，结论并入 §9.1 与各章。所有数字均为实测；**§3.0 起已用生产全量普查数据**（2026-08-23），**§7.5 第二轮是生产 `suite` 全量副本上的离线装载**（10,928 篇，2026-08-24），测量条件见 §10。
