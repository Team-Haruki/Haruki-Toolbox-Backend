...
func TestHandleUpdateRuntimeConfig(t *testing.T) {
	...
	if helper.PrivateAPIToken != "new-token" || helper.PrivateAPIUserAgent != "new-agent" {
		t.Fatalf("private api runtime config not updated: token=%q ua=%q", helper.PrivateAPIToken, helper.PrivateAPIUserAgent)
	}
	if helper.HarukiProxyUserAgent != "new-proxy-agent" || helper.HarukiProxyVersion != "v1.2.3" || helper.HarukiProxySecret != "new-proxy-secret" || helper.HarukiProxyUnpackKey != "new-unpack" {
		t.Fatalf("haruki proxy runtime config not updated")
	}
	if helper.WebhookJWTSecret != "new-webhook" {
