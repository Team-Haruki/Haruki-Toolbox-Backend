...
func parseUserTicketPositiveInt(raw string, defaultValue int, fieldName string) (int, error) {
...
func handleCreateOwnTicket(apiHelper *harukiAPIHelper.HarukiToolboxRouterHelpers) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		defer func() {
			writeUserAuditLog(c, apiHelper, "user.ticket.create", result, userID, map[string]any{
				"reason":   reason,
...
func handleListOwnTickets(apiHelper *harukiAPIHelper.HarukiToolboxRouterHelpers) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		page, err := parseUserTicketPositiveInt(c.Query("page"), defaultUserTicketPage, "page")
		if err != nil {
			if fiberErr, ok := err.(*fiber.Error); ok {
				return harukiAPIHelper.UpdatedDataResponse[string](c, fiberErr.Code, fiberErr.Message, nil)
			}
		...
		pageSize, err := parseUserTicketPositiveInt(c.Query("page_size"), defaultUserTicketPageSize, "page_size")
		if err != nil {
			if fiberErr, ok := err.(*fiber.Error); ok {
				return harukiAPIHelper.UpdatedDataResponse[string](c, fiberErr.Code, fiberErr.Message, nil)
			}
...
func registerTicketRoutes(apiHelper *harukiAPIHelper.HarukiToolboxRouterHelpers) {
	r := apiHelper.Router.Group("/api/user/:toolbox_user_id/tickets")
	r.Get("/", apiHelper.SessionHandler.VerifySessionToken, checkUserNotBanned(apiHelper), handleListOwnTickets(apiHelper))
	r.Post("/", apiHelper.SessionHandler.VerifySessionToken, checkUserNotBanned(apiHelper), handleCreateOwnTicket(apiHelper))
	r.Get("/:ticket_id", apiHelper.SessionHandler.VerifySessionToken, checkUserNotBanned(apiHelper), handleGetOwnTicketDetail(apiHelper))
	r.Post("/:ticket_id/messages", apiHelper.SessionHandler.VerifySessionToken, checkUserNotBanned(apiHelper), handleAppendOwnTicketMessage(apiHelper))
	r.Post("/:ticket_id/close", apiHelper.SessionHandler.VerifySessionToken, checkUserNotBanned(apiHelper), handleCloseOwnTicket(apiHelper))
}
